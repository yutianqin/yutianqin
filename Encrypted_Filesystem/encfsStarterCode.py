...
class EncFS(Operations):
    ...
    def getattr(self, path, fh=None):
        ...
        full_path = self._full_path(path)
        print(f'In getattr(): full_path: {full_path}')
        st = os.lstat(full_path)
        props = dict((key, getattr(st, key)) for key in ('st_atime', 'st_ctime',
                                                         'st_gid', 'st_mode', 'st_mtime', 'st_nlink', 'st_size',
                                                         'st_uid'))
        # TODO FINISH ME TO UPDATE THE st_size file to the size of the unencrypted content
        if full_path in self.openFiles:
            props["st_size"] = len(self.openFiles[full_path])
        else:
            if os.path.isfile(full_path):
                with open(full_path, 'rb') as file:
                    salt = file.read(16)  # Read the salt
                    contents = file.read()  # Read the rest of the file
                    print(f"getattr: contents: {contents}")
                    f = self.useFernet(salt)  # Create a Fernet instance with the salt
                    plain_text = f.decrypt(contents)  # Decrypt the contents
                    print(f"getattr: plain_text: {plain_text}")
                    props["st_size"] = len(plain_text)  # Update the size field
        return props
...
'''
    ...
    def open(self, path, flags):
        ...
        full_path = self._full_path(path)
        if full_path in self.openFiles:
            print(f'open: The file "{full_path}" is already open.')
            return -1
        if not os.path.exists(full_path):
            print(f'open: Error: file "{full_path}" does not exist.')
            return -1
        ...
        try:
            with open(full_path, 'rb') as encrypted_file:
                ...
                f = self.useFernet(salt)
                decrypted_text = f.decrypt(encrypted_data)
        except IOError:
            # Handle the error if the file cannot be opened
            print(f'open: Error: Could not open file "{full_path}".')
            return -1

        # Store decrypted data in memory
        self.openFiles[full_path] = bytes(decrypted_text)
    ...
    def create(self, path, mode, fi=None):
        full_path = self._full_path(path)
        try:
            os.close(os.open(full_path, os.O_CREAT | os.O_WRONLY, mode))
        except OSError:
            return -1
        self.openFiles[full_path] = ""
        new_fd = self.netFD
        self.netFD += 1
        return new_fd
    ...
    def read(self, path, length, offset, fh):
        ...
        """
        full_path = self._full_path(path)
        var = self.openFiles[full_path][offset: offset + length]
        print(f"read: var = {var}")
        return var
    ...
    def write(self, path, buf, offset, fh):
        """Write to a file."""
        full_path = self._full_path(path)
        var = self.openFiles[full_path]
        print(f"write(): var type: {type(var)}")  # str
        print(f"write(): buf type: {type(buf)}")  # bytes
        if isinstance(var, str):
            self.openFiles[full_path] = var[0: offset].encode() + buf
        else:
            self.openFiles[full_path] = var[0: offset] + buf
   ...
   '''
    ...
    def release(self, path, fh):
        ...
        """
        full_path = self._full_path(path)
        msg = self.openFiles[full_path][16:]
        print(f"release(): msg type: {type(msg)}")
        random_salt = os.urandom(16)
        f = self.useFernet(random_salt)
        cipher_text = f.encrypt(msg)

        var = random_salt + cipher_text
        print(f"release(): var type: {type(var)}")
        self.write(path, var, 0, None)
